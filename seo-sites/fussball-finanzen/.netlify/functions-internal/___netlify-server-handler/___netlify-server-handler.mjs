...
} from '/var/task/atlas-seo-blog/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/atlas-seo-blog/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/atlas-seo-blog')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/atlas-seo-blog',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/atlas-seo-blog/.netlify/dist/run/handlers/server.js')
